...
import { PaginatedData, StockActivityLog } from "@/types";
import { router } from "@inertiajs/react";
import { useDebouncedCallback } from "use-debounce";

interface ServerSideDataTableProps {
    columns: ColumnDef<StockActivityLog, any>[];
    paginatedData: PaginatedData<StockActivityLog>;
    searchValue?: string;
...
export function ServerSideDataTable({
    ...
    const debouncedSearch = useDebouncedCallback((value: string) => {
        ...
        if (value) {
            currentUrl.searchParams.set(searchParamName, value);
        } else {
            currentUrl.searchParams.delete(searchParamName);
        }

        // Reset to first page when searching
        currentUrl.searchParams.delete("stock_activity_page");
        ...
        <div className="grid auto-rows-max space-y-4">
            ...
            <div>
                <ServerSideDataTablePagination
                    ...
                    pageParamName="stock_activity_page"
                    perPageParamName="stock_activity_per_page"
                />
            </div>
        </div>
    );
